...
const SearchApp: React.FC = () => {
  ...
  const debouncedSearch = useCallback(
    _.debounce(async (search: string, threshold: number, fast: boolean, limit: number, speed: string, dataset: string): Promise<void> => {
      if (!search) {
        setResults([]);
  ...
  const handleSpeedChange = (newSpeed: string): void => {
    setSpeed(newSpeed);
  };
        ...
        <div className="bg-white rounded-xl shadow-xl p-8 mb-8 border border-gray-100">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              {/* Speed selection */}
              <div className="bg-gradient-to-br from-gray-50 to-gray-100 p-5 rounded-lg shadow-sm border border-gray-200 h-[170px]">
                <label className="block text-sm font-medium text-gray-700 mb-3">
                  Processing Speed
                </label>
                <div className="flex bg-gray-200 rounded-lg p-1 h-[110px]">
                  {['normal', 'fast', 'fastest'].map((speedOption) => (
                    <button
                      key={speedOption}
                      onClick={() => handleSpeedChange(speedOption)}
                      className={`flex-1 py-2 px-2 text-center rounded-lg transition-all duration-300 flex flex-col justify-center items-center ${
                        speed === speedOption 
                    ...
                    >
                      <div className="flex flex-col items-center justify-center h-full space-y-3">
                        ...
                        <span className="text-center">{speedOption.charAt(0).toUpperCase() + speedOption.slice(1)}</span>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              <div className="bg-gradient-to-br from-gray-50 to-gray-100 p-5 rounded-lg shadow-sm border border-gray-200 h-[170px]">
                ...
                <div className="grid grid-cols-2 gap-4 h-[110px]">
                  {[
                    ...
                    <label
                      key={value}
                      className={`flex flex-col items-center justify-center rounded-lg border-2 transition-all duration-300 cursor-pointer h-full ${
                        dataset === value
            ...
            <ul className="divide-y divide-gray-200">
              {results.map((result: SearchResult, index: number) => (
                <li key={index} className="py-5 transition-colors hover:bg-blue-50 rounded-lg px-4 group cursor-pointer">
                  <div className="flex items-center">
                    <div className="flex-shrink-0 h-10 w-10 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-bold group-hover:bg-blue-200 transition-colors">
                      {index + 1}
                    </div>
                    ...
                    {result.score !== undefined && (
                      <div className="ml-3 text-sm font-medium text-gray-500 group-hover:text-blue-600 transition-colors">
